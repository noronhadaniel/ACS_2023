...
class ProportionalController:
    ...
    def _fy(self, V, Cd_rocket, Cd_tabs, A_tabs, launch_angle, M_e):
        ...
        cross_sectional_area = (6.25 * 0.0254 / 2) ** 2 * math.pi  # [diamter in to m] [m**2]
        Ky = (
            -0.5 * rho * Cd_rocket * V ** 2 * cross_sectional_area * math.cos(launch_angle)
            - 0.5 * rho * Cd_tabs * V ** 2 * A_tabs * math.cos(launch_angle) - M_e * g
        ) / M_e
        return Ky
    ...
    def apogee_predict(self):
        ...
        flap_width = 1.75 * 0.0254  # [in to m] flap width
        flap_length = 5.5 * 0.0254  # [in to m] flap length
        mass = 773.95 / 35.274  # [oz to kg] EMPTY mass of rocket  # [m/s**2] gravity
        ...
        # Unused code
        # extension = math.sin(0) * flap_length
        extension = 0
        # Cd_o_tabs = 1.28*math.sin(0)
        Cd_o_tabs = 0
        # Cd_tabs = 1/math.sqrt(1-Mach**2)*Cd_o_tabs
        Cd_tabs = 0
        flaps_area = flaps_area = 4 * flap_width * flap_length
        Cd_rocket = 0.42
        ...
        H_sim = self._height
        while V_sim > 0:
            k1vy = fixed_dt * self._fy(V_sim, Cd_rocket, Cd_tabs, flaps_area, launch_angle, mass)
            k1ry = fixed_dt * V_sim
            
            k2vy = fixed_dt * self._fy(V_sim + 0.5 * k1vy, Cd_rocket, Cd_tabs, flaps_area, launch_angle, mass)
            k2ry = fixed_dt * (V_sim + k1vy/2)
            
            k3vy = fixed_dt * self._fy(V_sim + 0.5 * k2vy, Cd_rocket, Cd_tabs, flaps_area, launch_angle, mass)
            k3ry = fixed_dt*(V_sim + k2vy/2)
            
            k4vy = fixed_dt * self._fy(V_sim + k3vy, Cd_rocket, Cd_tabs, flaps_area, launch_angle, mass)
